...
class AppLocalizations {
  ...
  String? _runtimeOverrides(String key) {
    if (key == 'signup_title') {
      return switch (locale) {
        AppLocale.ru => 'Сформировать учебный путь',
        AppLocale.en => 'Build your learning path',
        AppLocale.kk => 'Оқу жолын қалыптастыру',
      };
    }
    ...
    switch (key) {
      case 'signup_title':
        return switch (locale) {
          AppLocale.ru => 'Сформировать учебную траекторию',
          AppLocale.en => 'Build your learning path',
          AppLocale.kk => 'Оқу траекториясын қалыптастыру',
        };
      case 'presentation_ready':
        return switch (locale) {
          AppLocale.ru => 'Структурированное обучение по IT-направлениям',
          AppLocale.en => 'Structured learning across IT paths',
          AppLocale.kk => 'IT бағыттары бойынша құрылымдалған оқу',
        };
      case 'reset_demo':
        return switch (locale) {
          AppLocale.ru => 'Прогресс обновлён',
          AppLocale.en => 'Progress refreshed',
          AppLocale.kk => 'Прогресс жаңартылды',
        };
      case 'faq_a_platform':
        return switch (locale) {
          AppLocale.ru =>
            'ZerdeStudy объединяет учебные ветки, курсы, AI-помощь и сохранение прогресса в одном приложении.',
          AppLocale.en =>
            'ZerdeStudy brings learning tracks, courses, AI help, and saved progress together in one app.',
          AppLocale.kk =>
            'ZerdeStudy оқу бағыттарын, курстарды, AI көмегін және прогресті бір қолданбада біріктіреді.',
        };
      case 'forgot_password_subtitle':
        return switch (locale) {
          AppLocale.ru =>
          ...
          AppLocale.kk =>
            'Email енгізіңіз, біз құпиясөзді қалпына келтіру кодын жібереміз.',
        };
      case 'reset_code_sent':
        return switch (locale) {
          AppLocale.ru =>
            'Если аккаунт найден, код для восстановления уже отправлен.',
          AppLocale.en =>
            'If the account exists, the reset code has already been sent.',
          AppLocale.kk =>
            'Егер аккаунт табылса, қалпына келтіру коды жіберілді.',
        };
      case 'verification_code_subtitle':
        return switch (locale) {
          AppLocale.ru =>
            'Введите 6-значный код, отправленный на {email}, и задайте новый пароль.',
          AppLocale.en =>
            'Enter the 6-digit code sent to {email} and set a new password.',
          AppLocale.kk =>
            '{email} мекенжайына жіберілген 6 таңбалы кодты енгізіп, жаңа құпиясөз орнатыңыз.',
        };
  ...
  _localizedValues = <String, Map<AppLocale, String>>{
    ...
    'tab_ai': {AppLocale.ru: 'ИИ', AppLocale.en: 'AI', AppLocale.kk: 'ЖИ'},
    'tab_profile': {
    ...
    'message_hint': {
      AppLocale.ru: 'Спросите про урок, практику, дерево знаний или статистику',
      AppLocale.en:
    ...
    'section_ai': {AppLocale.ru: 'AI', AppLocale.en: 'AI', AppLocale.kk: 'AI'},
    ...
    'catalog_empty_subtitle': {
      ...
      AppLocale.en: 'Try adjusting your search query or the selected filters.',
      AppLocale.kk:
    ...
    'you_label': {AppLocale.ru: 'Вы', AppLocale.en: 'You', AppLocale.kk: 'Сіз'},
    'frequent_qa_testing': {
    ...
    'course_enroll_body': {
      AppLocale.ru:
          'Откройте короткий onboarding и затем перейдите в мини-player с объяснением, кодом и заданиями.',
      AppLocale.en:
          'Open a short onboarding flow and then continue into the mini player with explanation, code, and tasks.',
      AppLocale.kk:
          'Қысқа onboarding ағынын ашып, кейін түсіндірме, код және тапсырмалары бар mini-player-ге өтіңіз.',
    },
    'course_start_now': {
      AppLocale.ru: 'Начать',
    ...
    'faq_subtitle': {
      AppLocale.ru:
          'Быстрые ответы о платформе, курсах, дереве знаний и сертификатах.',
      AppLocale.en:
          'Quick answers about the platform, courses, knowledge tree, and certificates.',
      AppLocale.kk:
          'Платформа, курстар, білім ағашы және сертификаттар туралы жылдам жауаптар.',
    },
    'faq_q_platform': {
    ...
    'faq_a_platform': {
      AppLocale.ru:
          'ZerdeStudy — это локальный demo-first учебный продукт с маршрутами, курсами, AI-помощью и сохранением прогресса без backend.',
      AppLocale.en:
          'ZerdeStudy is a local demo-first learning product with tracks, courses, AI help, and saved progress without a backend.',
      AppLocale.kk:
          'ZerdeStudy — backend-сіз де жұмыс істейтін, бағыттар, курстар, AI көмегі және сақталатын прогресі бар local demo-first оқу өнімі.',
    },
    'faq_q_tree': {
    ...
    'faq_a_tree': {
      AppLocale.ru:
          'Открывайте ветки из дерева, переходите в обзор трека и продолжайте обучение по урокам, практикам и тестам.',
      AppLocale.en:
          'Open branches from the tree, move into the track overview, and continue through lessons, practices, and assessments.',
      AppLocale.kk:
          'Ағаштағы тармақтарды ашып, трек шолуына өтіңіз де, сабақтар, практикалар және бағалаулар арқылы жалғастырыңыз.',
    },
    'faq_q_courses': {
    ...
    'faq_a_courses': {
      AppLocale.ru:
          'Tree показывает учебные ветки и фундаментальные направления, а Learn работает как витрина и каталог курсов.',
      AppLocale.en:
          'Tree shows learning branches and foundational paths, while Learn works as a course showcase and catalog.',
      AppLocale.kk:
          'Tree оқу тармақтары мен іргелі бағыттарды көрсетеді, ал Learn курс витринасы және каталогы ретінде жұмыс істейді.',
    },
    'faq_q_ai': {
    ...
    'faq_a_ai': {
      AppLocale.ru:
          'AI объясняет темы, помогает с кодом, подсказывает следующий шаг и отвечает прямо внутри некоторых экранов курса.',
      AppLocale.en:
          'AI explains topics, helps with code, suggests the next step, and answers directly inside some course screens.',
      AppLocale.kk:
          'AI тақырыптарды түсіндіреді, кодпен көмектеседі, келесі қадамды ұсынады және кейбір курс экрандарының ішінде бірден жауап береді.',
    },
    'faq_q_certificates': {
    ...
    'faq_a_certificates': {
      AppLocale.ru:
          'Сертификат появляется после завершения mini-course. Стандартный сертификат выдаётся от 70%, а premium — при 100% прохождения.',
      AppLocale.en:
          'A certificate appears after finishing a mini-course. Standard is awarded from 70%, and premium at 100% completion.',
      AppLocale.kk:
          'Сертификат mini-course аяқталғаннан кейін пайда болады. Standard 70%-дан бастап, ал premium 100% өткенде беріледі.',
    },
    'faq_q_profile': {
    ...
    'faq_a_profile': {
      AppLocale.ru:
          'В профиле собраны прогресс, достижения, сертификаты, избранное, завершённые материалы и история результатов.',
      AppLocale.en:
    ...
    'repeat_wrong_answers_hint': {
      AppLocale.ru:
          'У вас накопилось {count} ошибок в тестах и заданиях. Их можно быстро повторить из этого блока.',
      AppLocale.en:
          'You currently have {count} incorrect answers across quizzes and course tasks. You can review them quickly from here.',
      AppLocale.kk:
          'Сізде тесттер мен тапсырмалар бойынша {count} қате жауап жиналды. Оларды осы жерден жылдам қайталауға болады.',
    },
    ...
    'forgot_password_subtitle': {
      AppLocale.ru:
          'Введите email, и мы откроем мок-экран с кодом для демонстрации.',
      AppLocale.en:
          'Enter your email and we will open a mock verification flow for the demo.',
      AppLocale.kk:
          'Email енгізіңіз, содан кейін демо үшін коды бар mock verification flow ашылады.',
    },
    'send_code': {
    ...
    'verification_code_subtitle': {
      AppLocale.ru:
          'Введите 6-символьный код, отправленный на {email}. Для демо подойдёт любой код.',
      AppLocale.en:
          'Enter the 6-digit code sent to {email}. For this demo, any code will work.',
      AppLocale.kk:
          '{email} мекенжайына жіберілген 6 таңбалы кодты енгізіңіз. Демо үшін кез келген код жарайды.',
    },
